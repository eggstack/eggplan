...
if grep -RInE 'eggwork|eggsearch|eggbench|eggsact|tokio|reqwest|hyper|mcp|async.runtime' "$cargo_toml" "$crate/src"; then
  echo "provider SPI must not depend on sibling runtimes, transports, or acquisition" >&2
  exit 1
fi

if grep -RInE '\b(async[[:space:]]+fn|tokio::|reqwest::|std::process|Command::new)\b' "$crate/src"; then
